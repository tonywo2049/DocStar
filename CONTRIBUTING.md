...
三条均须 exit 0。`tests.py` 退出码契约：仅「引擎自有逻辑红」返回 1；对照红 / TDD 待建 /
golden 待重锁返回 0（详见 `tests.py` 的 `main()` 末尾注释）。
...
命令→顶层键契约表=`SKILL.md`「JSON output contract」节。

纯文档 typo 等 trivial 改动豁免本条。

## 4. 实质性文档 critic×2

新文档，或对既有文档新增 / 修改**结论 / 裁决 / 规格 / 关键数字**的更新，落盘 commit 前须由
**独立 agent（非作者）以证伪姿态审两轮**，七维度必查：事实 / 完整性 / 内部一致 / 上下游一致 /
过度设计 / 语态污染 / 可判定性。commit message 记 `critic×N`。

handoff / 台账 / 状态行等过程记录豁免文档 critic，改用机检（逐字引用、链接与 commit 存在性）。
代码改动的过度设计审挂在合并前的卡级 diff（有编译器 / 测试 / CI 作第一验证器），不另起文档 critic。
