...
[![tests](https://github.com/tonywo2049/DocStar/actions/workflows/tests.yml/badge.svg)](https://github.com/tonywo2049/DocStar/actions/workflows/tests.yml)
![Python 3.9–3.13](https://img.shields.io/badge/Python-3.9%E2%80%933.13-blue)
...
- **分层一致性检查**——引用级检查永远内置，跨类型政策按声明执行，四态判定不装绿
...
- **明确退出码合同**——`0` 命令成功 / `1` 查询未命中或 gate 命中 / `2` 用法配置错（fail-closed），可直接接 CI
...
- **机器可读是一等输出**——查询与分析命令加 `--json`；输出确定性、byte-stable，同输入同输出，可锁 golden、可做差分。两个 HTML 命令始终写文件。
...
DocStar 零安装——clone 下来按路径调用即可（见快速开始）；装后自检 `python3 tests.py --skip-slow` 应全绿。要让 agent 自动发现它，可把仓库注册为 skill（详见 [SKILL.md](SKILL.md)「安装与自检」）：

- **Codex 用户级**：`ln -s /path/to/DocStar "${CODEX_HOME:-$HOME/.codex}/skills/docstar"`
- **Codex 仓库级**：用 `git archive` 把已跟踪的发行树导出到 `<repo>/.agents/skills/docstar`，不要复制源仓 `.git`；`.agents` 已从语料扫描排除。命令见 [SKILL.md](SKILL.md)。
- **Claude Code 用户级**：`ln -s /path/to/DocStar "$HOME/.claude/skills/docstar"`
- **项目指令**：把 [AGENTS.md](AGENTS.md) 或 [SKILL.md](SKILL.md) 的文档撰写约定并入项目规则。

不便使用符号链接时可复制完整目录。工具位于被扫描项目内时，调用时显式传 `--corpus <文档目录>`，避免把工具说明当作业务语料。

卸载 = 逐个移除你创建过的落点：

- 注册过 skill → 删除对应的 Codex 或 Claude Code skill 链接/目录。
- 写作约定进过项目指令 → 这些约定是语料侧写作纪律、留不留随你，但其中指向工具命令的句子（如 `classify --pending`）须一并清掉，否则成死指引。
- 运行残留 → 工具目录树内的各 `__pycache__/`，以及调用目录中未显式给输出路径时生成的 `graph.html`/`entity_graph.html`。
- 不再要 clone 本体 → 删除 clone 目录；调用目录中的 HTML 产物按需另行删除。
...
docstar.py html [输出路径]             # 文档层交互关系图：力导向布局+搜索定位+详情面板
                                       #   不给路径→写在当前工作目录的 graph.html
docstar.py html-entity [输出路径]      # 实体层交互查询页：ego 邻域图+搜索+判定瓦片
                                       #   不给路径→写在当前工作目录的 entity_graph.html
```

通用旗标：`--json`（除 `html`/`html-entity` 外的查询与分析命令，输出机器可读 JSON）· `--corpus DIR`（扫描根，默认=当前目录）· `--conventions DIR`（显式指定 conventions 目录，覆盖自动发现）· `--include-archived`（任意命令，停用 conventions 声明的 archive_globs 归档过滤，取证查询用）。

退出码：`0` 命令成功（`check` 只有显式 `--gate` 才按检查结果改退出码）· `1` 查询未命中（含 `ids`/`dump` 的 `--kind` 未命中类别，stderr 列可选集合）或 `--gate` 命中 · `2` 用法或配置错误（fail-closed——conventions 缺必需 section、`--gate` 键名拼错、未知旗标，都不静默放过）。
...
给 agent 的入口和选命令规则在 [SKILL.md](SKILL.md)，精确参数与 JSON 顶层键在 [references/command-contracts.md](references/command-contracts.md)，配置和写作细则在 `references/`。仓库级协作规则见 [AGENTS.md](AGENTS.md)，贡献工序见 [CONTRIBUTING.md](CONTRIBUTING.md)。
