#!/usr/bin/env python3
"""tests.py — 实体图谱(eg-2) 断言运行器（波6-TB 从规格独立推导，推导基线 r12——r13/r14 为 additive 未改表A/B 行；纯 stdlib，DG-6）。
...
# ---------------- dump/edge 检索助手（设计 §3 eg-2 JSON 形状） ----------------
...
def a_harvest():
    ...
    ok("harvest/schema_algo", h.get("schema_version") == "eg-2" and h.get("algo") == "h1",
       "输出携 schema_version=eg-2、algo=h1（DG-9 字节可复现）")
...
def a_verdict_json():
    ...
    s2 = EM.emit({"k": dorm})
    ok("verdict/dormant_keeps_note", "说明" in s2 and "无声明" in s2,
       "休眠判定对象经 emit 保留「说明」反假绿信号（静默装绿=设计红线，DG-60）")
...
def a_contract_toplevel():
    ...
    EXPECTED_TOP = {
        ...
        "doc": ({"doc", "meta", "上游", "下游", "被引用frontmatter", "正文引出", "被正文引用",
                 "引出节引用", "被节引用", "节标题数", "ID提及TOP"}, ["doc", "A轨任务"]),
        "id": ({"id", "kind", "total", "docs"}, ["id", "R1-AC1"]),
        "id_section": ({"query", "目标锚点", "引用处"}, ["id", "A轨任务 §2"]),  # id "<doc> §N" 段引用形（处置 A-m5）
        "ids": ({"契约AC", "需求AC", "任务", "需求", "编号项", "版本", "参数", "决议", "里程碑", "审计AC"},
                ["ids"]),
        ...
        "check": ({"context_manifest", "fm_断链", "fm_无链接条目", "fm_有意非链接条目",
                   "单向边_我列它为下游_它未列我为上游", "单向边_我列它为上游_它未列我为下游",
                   "正文死链", "未登记参数_出现≥3次", "节引用前缀未解析TOP", "节引用断锚", "缺frontmatter",
                   "schema_version", "专名定义断锚", "CHK-2覆盖缺口", "CHK-2映射缺口", "CHK-3传导断裂",
                   "CHK-环检测", "unresolved_reference", "ambiguous_reference", "共现完备性",
                   "缺必需边", "未覆盖kind", "classification_complete", "实体_schema_孤儿consumer"},
                  ["check"]),
        "trace": ({"context_manifest", "query", "resolved", "性质", "primary", "candidates",
                   "attrs", "edges"}, ["trace", "C2-AC31"]),
        "brief": ({"context_manifest", "schema_version", "mode", "query", "resolved", "性质",
                   "judgment_status", "classification_complete", "truncated", "去重稳定排序",
                   "segments", "omitted", "diagnostics", "boundary_pointers", "tainted_by"},
                  ["brief", "TA2.3"]),
        "verify": ({"context_manifest", "schema_version", "baseline", "baseline_来源", "scan_root",
                    "引入实体", "删除实体", "引入边", "删除边", "引入缺陷", "进图缺失", "局限说明"},
...
def golden(name, cmd_args, delivered):
    ...
    if stale:
        rec(f"golden/{name}", "INFO",
            f"{gp.name} 为存量 eg-1/不可解析，待控制者波8 --bless 重锁 eg-2（本 agent 不碰 golden/）", "golden_wait")
        return
...
def layer_c():
    ...
    ok("slow/real_harvest_runs",
       rcode == 0 and isinstance(rharv, dict) and rharv.get("schema_version") == "eg-2",
    ...
    ok("slow/real_dump", rcode == 0 and isinstance(rdump, dict) and rdump.get("schema_version") == "eg-2",
       "自宿主 dump 跑通、schema=eg-2")
    ts = []
    for _ in range(5):
        t = time.perf_counter()
