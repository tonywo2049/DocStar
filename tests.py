...
def a_schema():
    ...
    ok("schema/default_kinds_generic",
       ...
       and set(EM.AUXILIARY_KINDS) == {"任务卡", "执行日志", "最新事件"}
       and not ({"契约AC", "审计AC", "评审项", "治理期权", "需求R"} & set(EM.DEFAULT_KINDS)),
       "DEFAULT_KINDS=7 内置通用词表；4 project-specific kinds 与 需求R 均不在内（EG-19-AC3）")
    ok("schema/edges_13_deleted", len(EM.EDGE_TYPES) == 13
       and all(x not in EM.EDGE_TYPES for x in ("定义于", "约束", "依据", "散文修订声明", "弱共现")),
       "EDGE_TYPES 13 类；定义于/约束/依据/散文/弱共现 5 边已删")
    ok("schema/edges_added", all(x in EM.EDGE_TYPES for x in
                                  ("阅读依赖", "前置依赖", "provenance", "共现索引",
                                   "执行卡", "执行日志", "最新事件")),
       "阅读依赖/前置依赖/provenance/共现索引/执行卡/执行日志/最新事件在册")
    ok("schema/execution_log_tokens",
       JC.TOKENS.get("任务卡") == "task-card" and JC.TOKENS.get("执行卡") == "execution-card"
       and JC.TOKENS.get("执行日志") == "execution-log"
       and JC.TOKENS.get("最新事件") == "latest-event",
       "task-card/execution-card/execution-log/latest-event 有稳定 eg-3 英文 mapping")
...
def a_bilingual_docs():
    ...
    tokens = ("locale", "purpose", "upstream", "downstream", "status", "type", "nature",
              ...
              "spec anchor", "prerequisite", "execution")
    ...
    canonical_task_header = "| # | task | spec anchor | prerequisite | status | execution |"
    task_surfaces = [
        writing_en,
        writing_zh,
...
def a_execution_cards():
    """GMGN Task→Card→Log→latest_event：正常链、休眠、坏链和 brief 最小覆盖。"""
    layer("logic")

    code0, dormant, _ = run_json("dump", corpus=EXECUTION_LOG)
    dormant_kinds = {e["key"][0] for e in (dormant or {}).get("entities", [])}
    ok("execution/config-absent-dormant",
       code0 == 0 and not ({"任务卡", "执行日志", "最新事件"} & dormant_kinds)
       and "执行日志诊断" not in (dormant or {}).get("reports", {}),
       "缺 task_execution 配置时执行关系完全休眠")

    code, dump, err = run_json("dump", "--preset", "gmgn-v1", corpus=EXECUTION_LOG)
    ok("execution/dump-runs", code == 0 and isinstance(dump, dict),
       f"gmgn-v1 新执行链 fixture 可导出：{err}")
    if not dump:
        return

    card = "execution/T1/Card.md"
    log = "execution/T1/Log.md"
    ok("execution/task-card-log-event-chain",
       bool(find_edge(dump, "执行卡", src_cid="T1", dst_cid=card))
       and bool(find_edge(dump, "执行日志", src_cid=card, dst_cid=log))
       and bool(find_edge(dump, "最新事件", src_cid=log, dst_cid="event-2")),
       "T1 建立 Task→Card→Log→latest_event 三跳关系")

    diagnostics = dump.get("reports", {}).get("执行日志诊断", [])
    ok("execution/none-is-clean",
       not any(item.get("task") == "T2" for item in diagnostics)
       and not find_edge(dump, "执行卡", src_cid="T2"),
       "execution=none 表示尚未物化，不报错也不成边")
    ok("execution/missing-card-explicit",
       any(item.get("task") == "T3" and item.get("reason") == "card_target_missing"
           for item in diagnostics),
       "缺失 Card 目标显式进入 diagnostics")
    ok("execution/missing-event-explicit",
       any(item.get("task") == "T4" and item.get("reason") == "latest_event_anchor_missing"
           for item in diagnostics),
       "缺失 latest_event 锚显式进入 diagnostics")

    brief_code, brief, brief_err = run_json(
        "brief", "T1", "--preset", "gmgn-v1", corpus=EXECUTION_LOG)
    source = "\n".join(str(segment.get("原文") or "")
                       for segment in (brief or {}).get("segments", []))
    ok("execution/brief-card-and-latest-only",
       brief_code == 0 and "CARD_TDD_CONTRACT" in source
       and "LATEST_EVENT_ONLY" in source and "OLD_EVENT_MUST_NOT_ENTER_BRIEF" not in source,
       f"execute brief 纳入规范 Card 与最新事件，不吞完整 Log：{brief_err}")

    bad_code, bad_brief, _ = run_json(
        "brief", "T3", "--preset", "gmgn-v1", corpus=EXECUTION_LOG)
    ok("execution/bad-chain-brief-explicit",
       bad_code == 0 and list_has((bad_brief or {}).get("diagnostics"), "card_target_missing")
       and list_has((bad_brief or {}).get("omitted"), "card_target_missing"),
       "坏执行链在 brief diagnostics 与 omitted 同时显式出现")

    trace_code, event_trace, _ = run_json(
        "trace", "event-2", "--preset", "gmgn-v1", corpus=EXECUTION_LOG)
    ok("execution/latest-event-queryable",
       trace_code == 0 and (event_trace or {}).get("resolved", [None])[0] == "最新事件",
       "latest_event 可独立 trace")

    task_ids = {e["key"][2] for e in dump.get("entities", []) if e["key"][0] == "任务"}
    ok("execution/canonical-task-table-only", "T1" in task_ids and "T9" not in task_ids,
       "gmgn-v1 任务实体只来自 canonical Task 表")

    import copy
    from conventions import PRESETS_DIR
    preset = json.loads((PRESETS_DIR / "gmgn-v1.json").read_text(encoding="utf-8"))
    with tempfile.TemporaryDirectory() as tmp:
        bad = copy.deepcopy(preset)
        bad["task_execution"]["card_fields"]["execution_log"] = "execution_log"
        cfg = Path(tmp) / "bad"
        cfg.mkdir()
        (cfg / "conventions.json").write_text(json.dumps(bad), encoding="utf-8")
        invalid_code, _out, invalid_err = run(
            "dump", "--conventions", str(cfg), corpus=EXECUTION_LOG)
        ok("execution/config-invalid-fail-closed",
           invalid_code == 2 and "task_execution" in invalid_err,
           "非法 task_execution 配置 fail-closed")

        without = copy.deepcopy(preset)
        without.pop("task_execution")
        ...
        ok("execution/conventions-hash-changes",
...
def main():
    ...
    a_execution_cards()  # GMGN Task→Card→Log→latest_event 图与 brief 分流
