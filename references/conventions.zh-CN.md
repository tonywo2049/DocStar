...
- 中英文正文都使用的标准任务表结构：`# | task | spec anchor | prerequisite | status |
  execution`；
- 任务实体只来自该标准任务表，不从重复的加粗字段标签或其他表格产生；
- 从 Task 的 `execution` 链接到 `execution/<card_id>/Card.md`，再从 Card 的
  `execution_log` 链接到同目录 `Log.md`，最后从 Log 的 `latest_event` 链接到事件锚，
  形成 `task → task-card → execution-log → latest-event`；
...
- `task_execution`：可选的 Card/Log 字段别名和 `canonical_task_table_only` 开关。缺席时执行
  关系抽取休眠；存在时必须有 `task_columns.execution`。Card 路径必须是
  `execution/<card_id>/Card.md`，声明 `type: task-card`、`nature: normative`；Log 必须是同目录
  `Log.md`，声明 `type: execution-log`、`nature: descriptive`；`latest_event` 必须解析到 Log 内锚。
  字段名可用 `=`、ASCII `:` 或中文 `：` 分隔。代码示例或 HTML 注释内链接不参与解析。非法
  声明会显式进入 `execution_log_diagnostics` 和 `brief` 的 omitted 清单。
